#!/usr/bin/env python3
"""
Build a self-contained <app> distribution (no Python required on the target) using PyInstaller,
then package it as
    dist/<app>-<version>-<os>-<arch>.zip      (Windows)
    dist/<app>-<version>-<os>-<arch>.tar.gz   (Linux / macOS, keeps exec bits)
...
APP_DESCRIPTION = "Develop digital photos: drop them in, press Start"   # Windows version resource
...
def write_version_info() -> str:
    ...
    with open(path, "w", encoding="utf-8") as fh:
        fh.write(VERSION_INFO_TEMPLATE % {
            ...
            "description": APP_DESCRIPTION, "copyright": copyright_line})
    return path
...
def pyinstaller_command() -> list:
    cmd = [
        ...
        "--collect-all=cv2", "--collect-all=rawpy",
        *metadata_flags("numpy", "requests", "packaging", "typing_extensions"),
